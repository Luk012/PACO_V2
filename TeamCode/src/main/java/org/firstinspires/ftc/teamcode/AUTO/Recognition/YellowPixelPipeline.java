...
public class YellowPixelPipeline extends OpenCvPipeline {
    ...
    Mat YCbCr = new Mat();
    Mat leftLeft_Cb, leftRight_Cb, centerLeft_Cb, centerRight_Cb, rightLeft_Cb, rightRight_Cb;
    Scalar rectColor = new Scalar(255.0, 0.0, 0.0); // red color in RGB - scalar object for the rectangle color on Camera Stream
    Mat Cb = new Mat();
    public int avg_leftLeft, avg_leftRight, avg_centerLeft, avg_centerRight, avg_rightLeft, avg_rightRight;
    ...
    public void init(Mat firstFrame) {
        ...
        Rect leftLeft = new Rect(1,2,3,4);
        Rect leftRight = new Rect(1,2,3,4);
        Rect centerLeft = new Rect(1,2,3,4);
        Rect centerRight = new Rect(1,2,3,4);
        Rect rightLeft = new Rect(1,2,3,4);
        Rect rightRight = new Rect(1,2,3,4);

        leftLeft_Cb = Cb.submat(leftLeft);
        leftRight_Cb = Cb.submat(leftRight);
        centerLeft_Cb = Cb.submat(centerLeft);
        centerRight_Cb = Cb.submat(centerRight);
        rightLeft_Cb = Cb.submat(rightLeft);
        rightRight_Cb = Cb.submat(rightRight);
...
//        Mat region3_bw = input.submat(rightRect);
        ...
        Imgproc.rectangle(input, leftLeft, rectColor, 2);
        Imgproc.rectangle(input, leftRight, rectColor, 2);
        Imgproc.rectangle(input, centerLeft, rectColor, 2);
        Imgproc.rectangle(input, centerRight, rectColor, 2);
        Imgproc.rectangle(input, rightLeft, rectColor, 2);
        Imgproc.rectangle(input, rightRight, rectColor, 2);

        Mat leftLeft_bw = input.submat(leftLeft);
        Mat leftRight_bw = input.submat(leftRight);
        Mat centerLeft_bw = input.submat(centerLeft);
        Mat centerRight_bw = input.submat(centerRight);
        Mat rightLeft_bw = input.submat(rightLeft);
        Mat rightRight_bw = input.submat(rightRight);

        avg_leftLeft = (int) Core.countNonZero(leftLeft_bw);
        avg_leftRight = (int) Core.countNonZero(leftRight_bw);
        avg_centerLeft = (int) Core.countNonZero(centerLeft_bw);
        avg_centerRight = (int) Core.countNonZero(centerRight_bw);
        avg_rightLeft = (int) Core.countNonZero(rightLeft_bw);
        avg_rightRight = (int) Core.countNonZero(rightRight_bw);

        leftLeft_bw.release();
        leftRight_bw.release();
        centerLeft_bw.release();
        centerRight_bw.release();
        rightLeft_bw.release();
        rightRight_bw.release();

        int[] averages = {avg_leftLeft, avg_leftRight, avg_centerLeft, avg_centerRight, avg_leftRight, avg_leftLeft};
        ...
        if(imx == 0) {
            ...
            whichSide = "centerLeft";
        }
        else if(imx == 3){
            whichSide = "centerRight";
        }
        else if(imx == 4){
            whichSide = "rightLeft";
        }
        else{
            whichSide = "rightRight";
        }
