...
@Config
@Autonomous(group = "Auto" , name = "BlueLeftNearWallDiff")

public class BlueLeftNearWallDiff extends LinearOpMode {
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            switch (status) {
                ...
                case VERIF:
                {
                    ...
                    if(collect.seconds() > 1.5 && (r.right_pixel.getState() == TRUE || r.left_pixel.getState() == TRUE) && ok2 == FALSE)
                    {
                        collectAngle.stack_level = Math.max(0, collectAngle.stack_level-1);
                        ok2 = TRUE;
                    }
